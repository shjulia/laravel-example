 ...
 */
class PracticeReviewTest extends TestCase
{
    ...
    public function testFinisherShifts()
    {
        ...
        $this->service->createReviewToPractice(clone $potentialProvider, $shift, $data);
        $shift = $this->shiftRepository->getById($shift->id);
        /** @var Review $review */
        $review = $shift->reviewFromProvider[0];
        $this->assertTrue($shift->isHasReviewFromProvider());
        $this->assertEquals($text, $review->text);
        $this->assertEquals($score, $review->score);
        $this->assertEquals($potentialProvider->reviews_to_practice_total + 1, $shift->provider->reviews_to_practice_total);
        $this->assertEquals(($potentialProvider->average_stars_to_practice * $potentialProvider->reviews_to_practice_total + $review->score
            ) / ($potentialProvider->reviews_to_practice_total + 1), $shift->provider->average_stars_to_practice);
        $this->assertEquals(($practice->average_stars * $practice->hires_total + $review->score
            ) / ($practice->hires_total + 1), $shift->practice->average_stars);
